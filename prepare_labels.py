...
#     max_dif = max(dif_xy)
#     give = max_dif * give + np.clip(dif_xy[::-1] - dif_xy, a_min=0, a_max=np.inf) / 2  
...
def get_annotations(annotations_path: Path, working_dir: Path, frames: dict, resolution: int = 512, zoom_in: bool = True) -> Tuple[np.ndarray, dict]: 
    ...
    for key, value in frame_dict.items(): 
        ...
        anno_path = str(Path("annotations") / Path(value["frame"]["file_path"]).name)
        cv2.imwrite(str(working_dir / anno_path), canvas) 
